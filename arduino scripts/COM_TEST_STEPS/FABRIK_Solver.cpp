    ...
    case 3:
      {
        ...
        if (outputOri[2].Cross(inputOri[2]).Mod() < minOffset || jointIndex > jointIndexNext)
        {
          outOriTemp[1] = outOriTemp[2].Cross(outputOri[0]).Normalize();
          outOriTemp[0] = outOriTemp[1].Cross(outOriTemp[2]);
        }
        else
        {
          switch (TypeOfJoints[jointIndexNext])
          {
            case 2://type II
              {
                outOriTemp[0] = outputOri[2].Cross(inputOri[2]).Normalize();
                if (outOriTemp[0].Dot(inputOri[0]) < 0)
                {
                  outOriTemp[0] = -1 * outOriTemp[0];
                }
            ...
            case 1://type I
              {
                outOriTemp[1] = inputOri[2].Cross(outputOri[2]).Normalize();
                if (outOriTemp[1].Dot(inputOri[1]) < 0)
                {
                  outOriTemp[1] = -1 * outOriTemp[1];
                }
...
FABRIK_Solver::FABRIK_Solver(Vector3d InitialPositionJoints[7], Vector3d InitialTargetPosition, Vector3d InitialTargetOrientation[3])
{
  for (int i = 0; i < NumberJoints; i++)
  {
    ...
    if (i > 0)
    {
      LengthLink[i - 1] = (PositionJoints[i] - PositionJoints[i - 1]).Mod();
    }
  }
  PositionBase = { 0, 0, 0 };
  for (int i = 0; i <= 6; i++)
  {
    OrientationLink[i][0] = { 1, 0, 0 };
    OrientationLink[i][1] = { 0, 1, 0 };
    OrientationLink[i][2] = { 0, 0, 1 };
  }
...
FABRIK_Solver::FABRIK_Solver()
{
  ...
  Vector3d targetPosition1 = { 0, 20, 280 };
  Vector3d targetOrientation[3];
  inputPositionJoints[0] = { 0, 0, 0 };
  inputPositionJoints[1] = { 0, 0, 35 };
  inputPositionJoints[2] = { 0, 0, 97 };
  inputPositionJoints[3] = { 0, 0, 151 };
  inputPositionJoints[4] = { 0, 0, 216 };
  inputPositionJoints[5] = { 0, 0, 278 };
  inputPositionJoints[6] = { 0, 0, 313 };
  targetOrientation[0] = { 1, 0, 0 };
  targetOrientation[1] = { 0, 1, 0 };
  targetOrientation[2] = { 0, 0, 1 };
  for (int i = 0; i < NumberJoints; i++)
  {
    PositionJoints[i] = inputPositionJoints[i];
    if (i > 0)
    {
      LengthLink[i - 1] = (PositionJoints[i] - PositionJoints[i - 1]).Mod();
    }
  ...
  PositionTarget = targetPosition1;
  OrientationLink[7][0] = targetOrientation[0];
  OrientationLink[7][1] = targetOrientation[1];
  OrientationLink[7][2] = targetOrientation[2];
  OrientationTarget[0] = targetOrientation[0];
  OrientationTarget[1] = targetOrientation[1];
  OrientationTarget[2] = targetOrientation[2];
}
